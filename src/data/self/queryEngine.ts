...
export function PERIOD(p) {
  ...
  var m = p.m1 - p.m0 + 1,
    label;
  if (p.m0 === 1 && p.m1 === 12) label = p.y + '年全年';
  else if (p.m0 === 1 && p.m1 === 6) label = p.y + '年上半年';
  else if (m === 3 && p.m1 % 3 === 0) label = p.y + '年Q' + p.m1 / 3;
  else if (m === 1) label = p.y + '年' + p.m0 + '月';
  else if (p.m0 === 1) label = p.y + '年1-' + p.m1 + '月';
  else label = p.y + '年' + p.m0 + '-' + p.m1 + '月';
  return {
    label: label,
    m: m,
    year: p.y,
...
export function createQueryEngine(state: QueryConfig) {
  ...
  function orgScopeText() {
    ...
    ORG_TREE.children.forEach(function (g) {
      ...
      if (cs.length === g.children.length) names.push(g.short);
      else
  ...
  return {
    compute,
    genDetails: genDetails as () => DetailRow[],
    orgSelArr,
    orgUnits,
    orgHeadText,
    orgScopeText,
