...
export const UNITS = {
  yanj: {
    ...
    margin: 8.4,
    ships: 21,
    seg: [['长江下游（沪苏通—武汉）', .34], ['长江中游（武汉—宜昌）', .22], ['长江上游（重庆段）', .16], ['干支直达 / 进出江', .18], ['短驳 / 港内倒驳', .10]]
  },
  hai: {
    name: '长航海运 · 沿海近洋',
    price: {
    ...
    margin: 16.9,
    ships: 7,
    seg: [['沿海 · 环渤海—华南', .34], ['沿海 · 华东内贸', .22], ['外贸近洋（东北亚/东南亚）', .28], ['内河转水 / 联运', .16]]
  }
};
export const RENT = {
  revMo: 0.12,
...
export const ORG_LEAVES = ORG_TREE.children.flatMap(parent => parent.children.map(leaf => ({...leaf,parent})));
