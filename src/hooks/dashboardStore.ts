...
try {
  ...
  if (Array.isArray(saved))
    recentShips = saved.filter((n) => shipRecords.some((s) => s.name === n)).slice(0, 6);
} catch {
  /* Storage is optional, including during server-side validation. */
}
export function rememberShip(name: string) {
  recentShips = [name, ...recentShips.filter((n) => n !== name)].slice(0, 6);
  try {
    localStorage.setItem('bulk-recent-ships', JSON.stringify(recentShips));
  } catch {
    /* Private mode. */
  }
}
export function searchShips(query: string) {
  ...
  if (state.shipQuery)
    shipRecords
      .filter((s) => s.name.includes(state.shipQuery))
      .slice(0, 3)
      .forEach((s) => rememberShip(s.name));
  notify();
}
